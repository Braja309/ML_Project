...
class ModelTrainer:
    ...
    def initiate_model_trainer(self)->ModelTrainerArtifact:
        try:
            ...
            logging.info(f"Spliting Training and Testing features")
            x_train,y_train,x_test,y_test = train_array[:,:-1],train_array[:,-1],test_array[:,:-1],test_array[:,-1]
            
            logging.info(f"Extracting model config file path")
            model_config_file_path = self.model_trainer_config.model_config_file_path

            logging.info(f"Initializing Moedel Factory using model config file : [{model_config_file_path}]")
            model_facotry = ModelFactory(model_config_path=model_config_file_path)

            base_accuracy = self.model_trainer_config.base_accuracy
            logging.info(f"Expected accuracy: {base_accuracy}")

            logging.info(f"Initiating operation model selection")
            best_model = model_facotry.get_best_model(X=x_train,y=y_train,base_accuracy=base_accuracy)

            logging.info(f"Best model found on training dataset {best_model}")

            logging.info(f"Extracting trained model list")
            grid_searched_best_model_list:list[GridSearchedBestModel] = model_facotry.grid_searched_best_model_list

            model_list = [model.best_model for model in grid_searched_best_model_list]
            logging.info(f"Evaluation all trained model both on training and testing dataset")
