...
def render_artifact_dir(req_path):
    ...
    #joining base and reuested path
    ...
    files = {os.path.join(abs_path,file): file for file in os.listdir(abs_path) 
            if "artifact" in os.path.join(abs_path,file)}
...
def predict():
    ...
    if request.method == 'POST':
        ...
        latitude = float(request.form['lalitude'])
        ...
        housing_predict = HousingPredictor(model_dir=MODEL_DIR)
        median_house_value = housing_predict.predict(X= housing_df)

        context = {
            HOUSING_DATA_KEY: HousingData.get_housing_data_as_dict(),
            MEDIAN_HOUSING_VALUE_KEY: median_house_value
        }
